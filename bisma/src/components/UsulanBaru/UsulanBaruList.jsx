            ...
            <tbody>
              {data.map((item, index) => (
                <tr key={index}>
                  ...
                  <td>{item.status}</td>
                  <td>
                    <button onClick={() => navigate(`/penelitian/usulan/edit/${item.id}`)} className="bg-blue-500 px-2 py-1 rounded-md text-white">
                      edit
                    </button>
                    <button onClick={() => navigate(`/penelitian/usulan/edit/${item.id}`)} className="bg-blue-500 px-2 py-1 rounded-md text-white">
                      detail
                    </button>
                    <button onClick={() => handleDelete(item.id)} className={`bg-red-500 px-2 py-1 rounded-md text-white ${item.status != 1 ? 'hidden' : ''}`}>
                      delete
                    </button>
                  </td>
                </tr>
